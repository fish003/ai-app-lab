...
const canonicalSkillPath = 'skills/investment-assistant/SKILL.md';
const officialRepository = 'https://github.com/volcengine/ai-app-lab';
const officialSkillPath = 'demohouse/personal-investment-assistant/skills/investment-assistant/SKILL.md';
const officialEntryUrl = `${officialRepository}/blob/main/${officialSkillPath}`;
...
用法：
  ...
  node scripts/print-public-skill-command.mjs \\
    ...
    [--skill-path skills/investment-assistant/SKILL.md] \\
...
function main() {
  ...
  if (process.argv.includes('--official')) {
    ...
    process.stdout.write(`帮我初始化个人投资助手：${officialEntryUrl}\n`);
    return;
  }
  ...
  if (officialRef) {
    ...
    process.stdout.write(`帮我初始化个人投资助手：${officialRepository}/blob/${officialRef}/${officialSkillPath}\n`);
    return;
  }
  ...
  process.stdout.write(`帮我初始化个人投资助手：${entryUrl}\n`);
}
