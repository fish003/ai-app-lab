...
const repository = 'https://github.com/3494036618-eng/personal-investment-assistant';
const tag = `v${packageJson.version}`;
const officialRepository = 'volcengine/ai-app-lab';
const officialProjectPath = 'demohouse/personal-investment-assistant';
const officialSkillPath = `${officialProjectPath}/skills/investment-assistant/SKILL.md`;
const officialUpstreamPath = `${officialProjectPath}/UPSTREAM.json`;
const temporaryRoot = fs.mkdtempSync(path.join(os.tmpdir(), 'investment-public-install-'));
...
async function fetchText(url) {
  const response = await fetch(url, {
    headers: { 'user-agent': 'personal-investment-assistant-release-verifier' },
  ...
  return tracked
    .map(({ mode, trackedPath }) => {
      const relativePath = prefix
        ? path.relative(prefix, trackedPath).split(path.sep).join('/')
        : trackedPath;
...
async function verifyOfficialMirror(officialRef, releaseCommit) {
  ...
  const releaseSkill = fs.readFileSync(
    path.join(checkout, 'skills', 'investment-assistant', 'SKILL.md'),
    'utf8',
  );
  ...
  assert.deepEqual(
    ...
    'AI App Lab 中的完整个人投资助手镜像与独立发行包不一致。',
  );
  ...
  assert.equal(
    command,
    `帮我初始化个人投资助手：https://github.com/${officialRepository}/blob/${officialRef}/${officialSkillPath}`,
  );
...
try {
  ...
  const npmInstall = npmInvocation(['--prefix', 'app', 'ci']);
  run(npmInstall.command, npmInstall.args, { cwd: checkout, capture: false });
  const npmVerify = npmInvocation(['run', 'verify']);
  run(npmVerify.command, npmVerify.args, { cwd: checkout, capture: false });
