...
export const CANONICAL_REPOSITORY = 'https://github.com/3494036618-eng/personal-investment-assistant';
const RELEASE_API = 'https://api.github.com/repos/3494036618-eng/personal-investment-assistant/releases/tags';
...
export function validateReleaseCheckout({
  ...
  const head = runGit(resolvedRoot, ['rev-parse', 'HEAD']).toLowerCase();
  const tagCommit = runGit(resolvedRoot, ['rev-parse', `${expectedTag}^{commit}`]).toLowerCase();
...
export async function validateImmutableRelease(report, fetchImpl = fetch) {
  const response = await fetchImpl(`${RELEASE_API}/${encodeURIComponent(report.tag)}`, {
    headers: { 'user-agent': 'personal-investment-assistant-checkout-validator' },
