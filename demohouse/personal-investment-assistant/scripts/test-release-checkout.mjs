...
const temporaryRoot = fs.mkdtempSync(path.join(os.tmpdir(), 'investment-release-checkout-'));
...
try {
  ...
  assert.equal(report.tag, 'v0.3.1');
  ...
  git(['remote', 'set-url', 'origin', 'https://github.com/wrong-owner/personal-investment-assistant.git']);
